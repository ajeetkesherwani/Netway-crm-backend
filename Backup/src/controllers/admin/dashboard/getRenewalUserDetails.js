...
exports.getRenewedUsersDetailsByFilter = catchAsync(async (req, res, next) => {
    ...
    const activeUsers = await User.find(matchUserQuery).select("_id name email createdAt").lean();
    ...
    const renewedPlans = await PurchasedPlan.find({
        userId: { $in: userIds },
        isRenewed: true
    }).select("userId planId startDate endDate isRenewed").lean();
    ...
    if (filterValue === "day") {
        aggregated = renewedUsers.reduce((acc, user) => {
            ...
            if (!acc[key]) acc[key] = { _id: { day, month: monthNum, year: yearNum }, totalUsers: 0, users: [] };
        ...
        aggregated = renewedUsers.reduce((acc, user) => {
            ...
            if (!acc[key]) acc[key] = { _id: { monthWeek, month: monthNum, year: yearNum }, totalUsers: 0, users: [] };
            acc[key].totalUsers += 1;
            acc[key].users.push(user);
            return acc;
        }, {});
        ...
        aggregated = renewedUsers.reduce((acc, user) => {
            ...
            if (!acc[key]) acc[key] = { _id: { month: monthNum, year: yearNum }, totalUsers: 0, users: [] };
            acc[key].totalUsers += 1;
            acc[key].users.push(user);
            return acc;
        }, {});
    ...
    return successResponse(res, `${filterValue}-wise renewed user details`, Object.values(aggregated));
});
