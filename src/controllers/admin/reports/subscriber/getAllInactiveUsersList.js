...
// Helper to fetch reseller/lco names
async function getResellerLcoName(createdFor, id) {
  let resellerName = null;
  let lcoName = null;
...
exports.getAllInactiveUsersList = catchAsync(async (req, res, next) => {
  ...
  // Base filter for inactive users
  ...
  if (searchField && searchValue) {
    ...
    if (directFieldMap[searchField]) {
      ...
      // 🧩 Find resellers matching the search
      ...
      // Find LCOs under those resellers
  ...
  // Get total filtered inactive users count
  const totalUsers = await User.countDocuments(filter);

  // Fetch filtered and paginated inactive users
  ...
  return successResponse(res, "Inactive users fetched successfully", {
    total: totalUsers,
    page: parseInt(page),
    limit: parseInt(limit),
    data: modifiedUsers,
