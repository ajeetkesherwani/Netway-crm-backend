...
exports.getNonAssignedTicketList = catchAsync(async (req, res, next) => {
    // Fetch open tickets
    const tickets = await Ticket.find({ status: "NonAssigned" })
        .populate({
            path: "assignToId",
            select: "name",
        })
    ...
    successResponse(res, "NonAssigned tickets fetched successfully", formattedTickets);
});