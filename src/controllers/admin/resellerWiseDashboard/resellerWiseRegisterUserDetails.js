...
exports.getResellerWiseRegisterUserDetails = catchAsync(async (req, res) => {
    ...
    // === WEEK WISE (7-day range within selected month) ===
    else if (filter === "week") {
        const startDate = new Date(selectedYear, selectedMonth, 1);
        const endDate = new Date(selectedYear, selectedMonth + 1, 0, 23, 59, 59);
        dateRange = { $gte: startDate, $lte: endDate };
    ...
    // ===  MONTH WISE (for full selected year) ===
    ...
    const data = await User.aggregate([
        { $match: { ...matchCondition, createdAt: dateRange } },
        { $group: groupStage },
        { $sort: { "_id": 1 } },
    ]);
    ...
    return successResponse(res, "Reseller-wise registration trend fetched successfully", {
        resellerId,
        filter,
        year: selectedYear,
