...
exports.getAssignedTicketList = catchAsync(async (req, res, next) => {
    // Fetch open tickets
    const tickets = await Ticket.find({ status: "Assigned" })
        .populate({
            path: "assignToId",
            select: "name",
        })
    ...
    successResponse(res, "assigned tickets fetched successfully", formattedTickets);
});