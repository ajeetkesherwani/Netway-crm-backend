...
exports.getFixedTicketList = catchAsync(async (req, res, next) => {
    // Fetch open tickets
    const tickets = await Ticket.find({ status: "Fixed" })
        .populate({
            path: "assignToId",
            select: "name",
        })
    ...
    successResponse(res, "fixed tickets fetched successfully", formattedTickets);
});